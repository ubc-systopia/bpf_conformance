# Prevail verifier plugin

This standalone adapter lets `bpf_conformance_runner` use an unmodified
Prevail executable as a verifier plugin. The runner supplies an ELF XDP
program; the adapter writes it to a temporary file and invokes Prevail with
first-failure reporting enabled.

Verifier plugin exit codes are:

- `0`: accepted
- `1`: rejected; stdout contains Prevail's verifier output
...
build/bin/bpf_conformance_runner \
  ...
  --plugin_path build/bin/prevail_plugin \
  --plugin_options "--prevail /path/to/prevail" \
  --verifier true \
  --xdp_prolog true \
  --elf true
```
