...
class temporary_file_t
{
  public:
    ...
    {
        auto pattern = (std::filesystem::temp_directory_path() / "alivio-conformance-XXXXXX.o").string();
...
bool
run_alivio(const options_t& options, const std::string& elf_path, process_result_t& result)
{
    int output_pipe[2];
    ...
    pid_t child = fork();
    if (child < 0) {
        std::cerr << "failed to fork Alivio: " << std::strerror(errno) << std::endl;
        close(output_pipe[0]);
        close(output_pipe[1]);
        return false;
    }
    if (child == 0) {
        ...
        std::cerr << "failed to invoke Alivio: " << std::strerror(errno) << std::endl;
        _exit(127);
    }
    ...
    while (waitpid(child, &status, 0) < 0) {
        if (errno != EINTR) {
            std::cerr << "failed to wait for Alivio: " << std::strerror(errno) << std::endl;
            return false;
        }
    }
